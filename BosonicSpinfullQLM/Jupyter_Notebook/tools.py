...
def info_sym(Lx, Ly, Nu, Nd, tau, mass, BC, kx, ky, z, px, py, pd, ext = '.hdf5'):
    name = info_raw(Lx, Ly, Nu, Nd, tau, mass)
    if BC:  name += ",kx=%d,ky=%d"%(kx,ky)
    if Nu == Nd:  name += ",z=%d"%z
    if not BC or kx == 0 or kx == Lx / 2:  name += ",px=%d"%px
    if not BC or ky == 0 or ky == Ly / 2:  name += ",py=%d"%py
    if Lx == Ly and kx == ky and px == py:  name += ",pd=%d"%pd
    return name + ext
