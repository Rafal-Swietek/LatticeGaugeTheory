...
namespace QHS{
	...
	class Symmetric2DLattice : public hilbert_space_base<PackedLattice2D>
	{
		...
		void create_two_particle_basis();
    ...
    public:
		...
		std::pair<std::size_t, std::size_t> move(std::size_t x, std::size_t y, Direction dir) const;
		...
		arma::SpMat<elem_ty> symmetry_rotation() const { return arma::SpMat<elem_ty>(); }
		arma::SpMat<elem_ty> symmetry_rotation(const QHS::Symmetric2DLattice<_particle_type>&) const;
	...
	inline
	arma::SpMat<elem_ty>
	Symmetric2DLattice<_particle_type>::symmetry_rotation(const QHS::Symmetric2DLattice<_particle_type>& _hilbert_full) const
	{
		const u64 dim_tot = _hilbert_full.get_hilbert_space_size();
		arma::SpMat<elem_ty> U(dim_tot, this->dim);
	#pragma omp parallel for
		for (long int k = 0; k < this->dim; k++) {
			for (auto& G : this->_symmetry_group) {
				auto [Gstate, chi_g] = G(this->mapping[k]);
				u64 idx = _hilbert_full.find(Gstate);
			#ifdef USE_REAL_SECTORS
				if(idx < dim_tot) // only if exists in sector
					U(idx, k) += std::real(chi_g / (this->_normalisation[k]) );
			#else
				if(idx < dim_tot) // only if exists in sector
					U(idx, k) += std::conj(chi_g) / (this->_normalisation[k]);
			#endif
	...
	void Symmetric2DLattice<_particle_type>::create_basis()
	{
		...
		this->index_of.clear();
		this->dim = 0;
		if (this->_include_U1U1 && (this->_Nup == 1 && this->_Ndown == 1)) {
			this->create_two_particle_basis();
			return;
		}
		else if (this->_include_U1U1 && (this->_Nup + this->_Ndown < 4)) {
	...
	#pragma omp critical
		...
		// 	}
		// );
		arma::vec DimerNumber(this->dim);
		for(long alfa = 0; alfa < this->dim; alfa++)
		{
			auto base_state = this->mapping[alfa];
			for(std::size_t y = 0; y < this->_Ly; y++){
				for(std::size_t x = 0; x < this->_Lx; x++){
		...
		auto permut = sort_permutation2(
			this->mapping,
			[&](std::size_t i, std::size_t j)
			{
				if (DimerNumber(i) != DimerNumber(j))
					return DimerNumber(i) < DimerNumber(j);
				return this->mapping[i] < this->mapping[j];
			}
		);
	...
	template <QOps::particle _particle_type>
	inline
	std::pair<std::size_t, std::size_t> Symmetric2DLattice<_particle_type>::move(std::size_t x, std::size_t y, Direction dir) const
	{	
		//<! Adding this->_L (x,y) to avoid negative numbers when wrapping around boundary
		switch (dir) {
			case Direction::Right:
				return std::make_pair((x+this->_Lx + 1)%this->_Lx, y);
			case Direction::Left:
				return std::make_pair((x+this->_Lx - 1)%this->_Lx, y);
			case Direction::Up:
				return std::make_pair(x, (y+this->_Ly + 1)%this->_Ly);
			case Direction::Down:
				return std::make_pair(x, (y+this->_Ly - 1)%this->_Ly);
		}
		return std::make_pair(this->_Lx, this->_Ly);
	}
	...
	*/
	...
	template <QOps::particle _particle_type>
	inline
	void Symmetric2DLattice<_particle_type>::create_two_particle_basis()
	{
		this->mapping.clear();
		this->_normalisation.clear();
		this->index_of.clear();
		...
		auto add_state = [&](PackedLattice2D &state)
		{
			elem_ty norm;
			if (this->is_SEC_representative_and_norm(state, norm)) {
				this->mapping.push_back(state);
				this->_normalisation.push_back(norm);
			}
		};

		for (int s = 0; s < this->system_size; ++s)
		{
			// right neighbour
			if (int r = this->move(s, Direction::Right); r >= 0 && r != s)
			{
				// ----- dimer -----
				{
					PackedLattice2D state(this->_Lx, this->_Ly);
					...
					auto [xs, ys] = idx_to_xy(s);
					auto [xr, yr] = idx_to_xy(r);

					state.set_block(xs, ys, DIMER_RIGHT);
					state.set_block(xr, yr, DIMER_LEFT);
				...
				// ----- neighbouring monomers -----
				{
					...
					state.set_block(xs, ys, UP_MONOMER);
					state.set_block(xr, yr, DOWN_MONOMER);

					add_state(state);

					// if up/down are distinguishable, also include the swapped state
					state.set_block(xs, ys, DOWN_MONOMER);
					state.set_block(xr, yr, UP_MONOMER);
			...
			// up neighbour
			if (int r = this->move(s, Direction::Up); r >= 0 && r != s)
			{
				// ----- dimer -----
				{
					PackedLattice2D state(this->_Lx, this->_Ly);
					...
					state.set_block(xs, ys, DIMER_UP);
					state.set_block(xr, yr, DIMER_DOWN);
