...
{   
    ...
    this->_hilbert_space = QHS::Symmetric2DLattice<QOps::particle::fermion>(
...
FermionicQLM::FermionicQLM(int _BC, unsigned int Lx, unsigned int Ly, double tau, double m, SymmetrySectors syms_in, bool use_syms)
{ 
    CONSTRUCTOR_CALL;
...
{   
    ...
    //     this->symmetry_generators.emplace_back(QOps::_mirror_X_symmetry<QOps::particle::fermion>(this->Lx, this->Ly, this->syms.px_sym));
    // if(is_ky_real_sector)
    //     this->symmetry_generators.emplace_back(QOps::_mirror_Y_symmetry<QOps::particle::fermion>(this->Lx, this->Ly, this->syms.py_sym));
    ...
    if( (this->Lx == this->Ly) && (this->syms.kx_sym == this->syms.ky_sym) && this->syms.px_sym == this->syms.py_sym )
        this->symmetry_generators.emplace_back(QOps::_mirror_diagonal_symmetry<QOps::particle::fermion>(this->Lx, this->Ly, this->syms.pd_sym));
    
    if(this->syms.Nup == this->syms.Ndown)
        this->symmetry_generators.emplace_back(QOps::_spin_inversion_symmetry<QOps::particle::fermion>(this->Lx, this->Ly, this->syms.z_sym));
}
...
typename FermionicQLM::sparse_matrix FermionicQLM::create_local_hamiltonian(int site)
{
    ...
    auto pair_sign = [&](const PackedLattice2D& s, int a, int b) {
        if (a > b) std::swap(a, b);

        int parity = 0;
        for (int ell = a + 1; ell < b; ++ell) {
            parity ^= static_cast<int>( s.get_block(static_cast<std::size_t>(ell)) < 2 );
        }
            ...
            for (QHS::Direction dir : {QHS::Direction::Right, QHS::Direction::Up}) {
				...
				if (nei >= 0) 
                {
                    ...
                    if( (_bitj == UP_MONOMER && _bitnei == DOWN_MONOMER)
                        ...
                        double sign = pair_sign(base_state, site, nei);
                        set_loc_hamiltonian_elements(k, -this->_tau * sign, state);
